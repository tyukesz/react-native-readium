...
| `hidePageNumbers`    | `boolean`          | :white_check_mark: | Either to show or hide the page numbers at the bottom of the eBook (iOS only) |
...
| `onTableOfContents` | `(payload: TableOfContentsPayload) => void` | :white_check_mark: | A callback that fires once the file is parsed and emits the table of contents embedded in the file along with the total number of discrete positions (if available). Returns `null` or an empty `[]` for the TOC portion when none exists. See the [`Link`](https://github.com/5-stones/react-native-readium/blob/main/src/interfaces/Link.ts) interface for more info. |

```ts
type TableOfContentsPayload = {
  toc: Link[];
